...
@api_view(['POST'])
@permission_classes([IsAuthenticated]) # Ensures only logged-in users can generate tokens
def get_voice_token(request):
    """
    Proxies a request to OpenAI to create an ephemeral Realtime session.
    """
    
    # 1. Define the OpenAI Endpoint
    url = "https://api.openai.com/v1/realtime/client_secrets"
    
    # 2. Set headers (Use your server-side environment key here)
    headers = {
        "Authorization": f"Bearer {os.getenv('OPENAI_API_KEY')}",
        "Content-Type": "application/json",
    }
    
    # 3. Define Body (Model and Voice settings)
    payload = {
        "expires_after": {
            "anchor": "created_at",
            "seconds": 600
        ...
        "session": {
          "type": "realtime",
          "model": "gpt-realtime",
        #   "voice": "verse", # You can set a default voice here
        }
    }
    ...
    try:
        ...
        # 5. Return the JSON (containing client_secret) to your Frontend
        data = response.json()
        return Response(data)

    except requests.exceptions.RequestException as e:
        print(f"OpenAI API Error: {e}")
        return Response({"error": "Failed to generate voice token"}, status=500)
